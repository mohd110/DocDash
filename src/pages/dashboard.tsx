...
function NextUpHero({ appointment }: { appointment: AppointmentWithPatient }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-bottle-600 text-cream-100 shadow-lift">
      <div className="flex flex-col gap-6 p-6 sm:p-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-5">
          <div className="flex size-16 shrink-0 items-center justify-center rounded-2xl bg-cream-100/15 font-display text-xl font-semibold">
            {initials(patient?.full_name)}
          </div>
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-cream-100/70">
            ...
            <p className="mt-3 inline-flex items-center gap-2 rounded-full bg-cream-100/15 px-3.5 py-1.5 text-sm font-semibold">
              <Clock3 className="size-4" />
              {formatTime(appointment.scheduled_at)} · {relativeToNow(appointment.scheduled_at)}
            </p>
            {appointment.reason && (
              <p className="mt-3 max-w-lg text-sm leading-relaxed text-cream-100/85">
                “{appointment.reason}”
              </p>
            )}
          </div>
    ...
    <div className="mx-auto max-w-6xl space-y-7">
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display text-2xl font-semibold text-bottle-800">Today’s schedule</h2>
